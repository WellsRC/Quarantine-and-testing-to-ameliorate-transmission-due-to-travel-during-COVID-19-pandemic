function [rec,N] = RecoveredPopulationLI(Country,DateN,Demo,pA,AL)
...
findx=find(t&fDate);
Symptomatic=sum(T.confirmed_infections((findx-11):(findx))); % Distribute the number of infections over this past time based on immunity level
SymptomaticNoIsolation=sum(T.confirmed_infections((findx-31):(findx))); % Distribute the number of infections over this past time based on immunity level
Asymptomatic=sum(T.confirmed_infections((findx-31):(findx))); % Distribute the number of infections over this past time based on immunity level
