...
load('MLE-Estimate-RTPCR-Hill_Incubation_11_7_days.mat','beta','TPtID','TDate','TResult','PtID','par');
TI=par(1:end-2);
...
S = TestSensitivity(t,11.6616,2.9,inf,[],beta);
LL1=plot(t,S,'k','LineWidth',2);
hold on
plot(11.6616.*ones(101,1),linspace(0,1,101),':','color',[0.75 0.75 0.75],'LineWidth',2);
...
text(-6.30407911001236,0.995,'G','Fontsize',32,'FontWeight','bold');

subplot('Position',[0.563592436974789,0.5917,0.424600840336135,0.39]);

load('MLE-Estimate-RTPCR-Hill_Incubation_11_7_days.mat','beta');
betaPCR=beta;
load('BD Veritor_LR_Parameters.mat','beta');

t=linspace(0,50,1001);
S = TestSensitivity(t,11.6616,2.9,inf,[],betaPCR);
SAg = TestSensitivity(t,11.6616,2.9,inf,beta,betaPCR);
...
text(-6.30407911001236,0.995,'H','Fontsize',32,'FontWeight','bold');

print(gcf,'Figure_Diagnostic_Sensitivity_Longer.png','-dpng','-r600');