...
cFile={'NatComm-Quarantine_BDVeritor_Entry_Exit_Duration'};
...
for ii=1:NM
    for jj=(ii+1):NM
        ...
        if(~isempty(prevA))
            ...
            [qA,qB] = DetermineQuarantine(qR,nageA,nageB,[1],[1],[0],[0],[0],RA,RB,pA,d,prevA,prevB,vacA,vacB,recA,recB,cA,cB,vAB,vBA,dAB,dBA,NA,NB,cFile);
            if(~isempty(qA))
                QM(ii,jj)=qA;
            else
                QM(ii,jj)=15;           
...
print(gcf,['NatComm-Figure_Country_No_VOC_Dual_Ag_Test.png'],'-dpng','-r600');
print(gcf,['FigureS17.png'],'-dpng','-r600');
save('NatComm-Dual_Ag_Test_Country.mat');