function [RIEQA]=CalculateInequalityVOCAll(nageA,nageB,FVOCA,FVOCB,RVOC,REPSVOC,RNIVOC,pA,prevAw,prevBw,vacAw,vacBw,recAw,recBw,cAw,vAB,vBA,dAB,dBA,NA,NB,qAI,AL,cFile)
...
for jj=1:length(FVOCA)
    ...
    T2A=sum((1-(qAI+dAB).*vAB).*(cA.*nageA.*(1-phiAw)./(1-phiA)).*RA).*(1-phiA);  % Second component of inequality
