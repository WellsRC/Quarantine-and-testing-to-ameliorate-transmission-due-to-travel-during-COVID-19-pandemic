...
for nn=4:-1:4
    ...
    for ii=1:NM-1
        for jj=(ii+1):NM        
            [prevA,prevB,vacupA,vacupB,proHA,proHB,recA,recB,cA,cB,NA,NB,avgdAB,pgeoAB,VTAB,avgdBA,pgeoBA,VTBA,CAstatusR,~,VacupA,~,VOCBetaGH501YV2A,~,VOCAlpha20201201GRYA,~,VOCDeltaG478KV1A,~,DemoA,DemoB] = CountryDataReturnIncubationAirline(DateInter,CountryM(ii),CountryM(jj),pA,AL,IncubationP);
    ...
    save(['Country_Data_' DateInter{1} '_Incubation=' num2str(IncubationP) '_Adherence_Level_' num2str(nn*25) '_AIRLINEMATRIX.mat'],'prev','rec','c','N','vac','proH','avgABM','pgeoABM','VTABM','CountryM','pA','cstatusR','Vacup','VOCBetaGH501YV2','VOCAlpha20201201GRY','VOCDeltaG478KV1','Demo')
end