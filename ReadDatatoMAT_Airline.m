...
DateInter={'November 21, 2021'};
    ...
    VTABM=-1.*ones(NM,NM);
     ...
     for ii=1:NM-1
        for jj=(ii+1):NM        
            [prevA,prevB,vacupA,vacupB,proHA,proHB,recA,recB,cA,cB,NA,NB,avgdAB,pgeoAB,VTAB,avgdBA,pgeoBA,VTBA,CAstatusR,~,VacupA,~,VOCOmincronA,~,VOCDeltaA,~,DemoA,DemoB] = CountryDataReturnIncubationAirline(DateInter,CountryM(ii),CountryM(jj),pA,AL,IncubationP);
    ...
    save(['Country_Data_' DateInter{1} '_Incubation=' num2str(IncubationP) '_Adherence_Level_' num2str(100) '_AIRLINEMATRIX.mat'],'prev','rec','c','N','vac','proH','avgABM','pgeoABM','VTABM','CountryM','pA','cstatusR','Vacup','VOCOmincron','VOCDelta','Demo')
