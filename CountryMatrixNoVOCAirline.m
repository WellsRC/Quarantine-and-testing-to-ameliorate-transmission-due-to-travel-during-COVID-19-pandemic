function CountryMatrixNoVOCAirline(cFile,DateI,AL,AQ,IncubationP)
    ...
    for ii=1:NM
        for jj=(ii+1):NM
            [nageA,nageB,prevA,prevB,vacA,vacB,~,~,recA,recB,cA,cB,NA,NB,~,VTAB,dAB,~,VTBA,dBA,pA,~,~,~,~] = DataReturnSimAirline(CountryM(ii),CountryM(jj),AL,DateI,IncubationP);
            if(~isempty(prevA))
...
%                 end
    ...
    save(['Country_NO_VOC_Quarantine_' cFile '_AL=' num2str(AL*100) '_AQ=' num2str(100*AQ) '_AIRLINE_' DateI{1} '.mat']);
    
    PlotQuarantineMatrix(CountryM,QM,CSR)
    print(gcf,['Figure_Country_NoVOC_' cFile '_Adherence=' num2str(100*AL) '_AQ=' num2str(100*AQ) '_AIRLINE_' DateI{1} '.png'],'-dpng','-r600');
