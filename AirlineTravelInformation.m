function [TravelDepart,DoS,pgeo] = AirlineTravelInformation(CountryLeaving,CountryEntering)
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% https://ec.europa.eu/eurostat/databrowser/view/AVIA_PAOCC__custom_761331/default/table?lang=en
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
load([pwd '\Country_Data\Airline_Flow_Data.mat']);
...
elseif(strcmp(CountryLeaving,'United Kingdom'))
    ...
    elseif(strcmp(CountryEntering,'Sweden'))
         ...
         tB=strcmp(CountryLeaving,T.Country);
         %TravelDepart=(T.Total2019(tB)./365)./DoS; % Need to divide by duration of stay as we do not have avialble information for number of travellers
         x=[1:30];     
         if((~isempty(DoS))&&(~isnan(DoS)))
            pgeo=fmincon(@(z)(sum(x.*((z.*(1-z).^(x-1))./(1-(1-z)^30)))-DoS).^2,1./DoS,[],[],[],[],0,1);
        else
