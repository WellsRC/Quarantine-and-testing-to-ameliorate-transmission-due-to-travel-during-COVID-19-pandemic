clear;
clc;
load('Country_Data_April_12_2021.mat')

tempV=VTABM./repmat(N,1,31);
    ...
    for jj=1:NM
        ...
        if(~isempty(prevA))
            ...
            for pp=1:16
                ...
                vAB=(VTAB./NA);
                vBA=(VTBA./NB);
                [qA,~] = DetermineQuarantine(qR,nageAtemp,nageBtemp,[1],[1],[0],[0],[0],RA,RB,pA,d,prevA,prevB,vacA,vacB,recA,recB,cA,cB,vAB,vBA,dAB,dBA,NA,NB,cFile);
                if(~isempty(qA))
                    QM(pp,ii,jj)=qA;
                else
                    QM(pp,ii,jj)=15;           
                ...
                [qA,~] = DetermineQuarantine(qR,nageAtemp,nageBtemp,[1],[1],[0],[0],[0],RA,RB,pA,d,prevAtemp,prevBtemp,vacAtemp,vacBtemp,recAtemp,recBtemp,cAtemp,cBtemp,vABtemp,vBAtemp,dABtemp,dBAtemp,NAtemp,NBtemp,cFile);
                if(~isempty(qA))
                    QMSA(pp,ii,jj)=qA;
                else
                    QMSA(pp,ii,jj)=15;           
